...
class ShuffleFeature extends Feature {
  get featureKey() {
    return "features.true_shuffle";
  }
  ...
  get settingsFields() {
    return [
      {
        type: "select",
        key: "features.true_shuffle_mode",
        label: "Mode",
        options: [
      ...
      if (
        ...
        fetch(this._scUrl, {
          ...
          .then(async (data) => {
            ...
            if (
              SCLIENT_CONFIG.trueShuffle &&
              SCLIENT_CONFIG.trueShuffleMode === "api" &&
              data.tracks &&
  ...
  async onShuffleClick(e) {
    ...
    if (shuffleBtn && !shuffleBtn.classList.contains("m-shuffling")) {
      ...
      await this.forceLoadQueue();
      shuffleBtn.click();
    }
  }

  async forceLoadQueue() {
...
const SHUFFLE_FEATURE = new ShuffleFeature();
FEATURES.push(SHUFFLE_FEATURE);
