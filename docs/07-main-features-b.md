...
- featureKey: the stats feature is controlled by TWO keys (`stats.api_sync`, `stats.local_tracking`). Set `featureKey` to `stats.local_tracking` (used for enablement) and expose `get apiSyncEnabled() { return !!SCLIENT_CONFIG.statsApiSync; }`. Category `stats`, label "Listening Stats", hasToggle true (toggle reflects local_tracking). settingsCustom(): an "Open Stats" button (Phase 10 wires it).
